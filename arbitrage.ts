  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < data.length; j++) {
      ...
      if (Math.abs(callPutDiff) > 0) {
        ArbPercent = callPutDiff / item2.futuresPrice;
      }

      // Add arbitrage
      if (ArbPercent > MIN_ARB_RATE) {
        const resObj = {
          ...
          ArbDollarUSD: `${Number(callPutDiff.toFixed(3)).toLocaleString()} USD`,
          ArbPercent: `${(ArbPercent * 100).toFixed(3)}%`,
          TotalCost: `${((callPrice + putPrice) * 100).toLocaleString()} USD`,
